...
async def retry_with_flood_handling(func: Callable[..., T], action: str, *args, **kwargs) -> T:
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.warning(
                f"Hit FloodWait in {action}, waiting {delay} seconds before retry {attempt + 1}/{max_retries}"
            )
    ...
    **kwargs,
    ...
    while len(messages) < total_limit:
        try:
            ...
            logger.error(
                f"Error in get_chat_history: {e}, chat_id={chat_id}, limit={limit}, offset={offset}, offset_id={offset_id}, kwargs={kwargs}"
            )
            raise
        ...
        try:
            ...
            async for msg in mcp.client.search_messages(
                ...
                **{
                ...
                },  # Pass other kwargs except limit
            ):
                chunk.append(msg)
            ...
            logger.error(
                f"Error in search_messages: {e}, chat_id={chat_id}, query={query}, kwargs={kwargs}"
            )
            raise
...
class TelegramBridge(FastMCP):
    ...
    async def _start_client(self):
        ...
        try:
            ...
            await retry_with_flood_handling(self.client.start, "start_client")
    ...
    async def _run_with_transport(self, transport: Literal["stdio", "sse"]):
        ...
        try:
            if transport == "stdio":
                ...
                await asyncio.gather(client_task, super().run_sse_async())
        finally:
...
async def get_messages(
    ...
    try:
        ...
        logger.debug(
            f"Getting chat history with: id={chat_obj.id}, type={chat_obj.type}, limit={limit}, offset={parsed_offset}"
        )

        messages_list = await get_chat_history_with_retry(
            chat_id=chat_obj.id, limit=limit, offset=0, offset_id=parsed_offset
        )
...
async def search_messages(
    query: str, chats: List[str] | None = None, limit: int = 10, ctx: Context | None = None
) -> str:
    ...
    for chat in chats:
        try:
            ...
            messages_list = await search_messages_with_retry(
                chat_id=chat_obj.id, query=query, limit=limit
            )
...
@mcp.tool()
async def get_full_chat_info(chat: str | None = None, ctx: Context | None = None) -> str:
    ...
    try:
        ...
        base_info = {
            ...
            "has_protected_content": chat_obj.has_protected_content
            if hasattr(chat_obj, "has_protected_content")
            else False,
        ...
        if str(chat_obj.type) in ["supergroup", "channel"]:
            info = {
                **base_info,
                ...
                "description": ensure_unicode(chat_obj.description)
                if hasattr(chat_obj, "description")
                else None,
                "member_count": chat_obj.members_count
                if hasattr(chat_obj, "members_count")
                else None,
                "is_restricted": chat_obj.is_restricted
                if hasattr(chat_obj, "is_restricted")
                else False,
            ...
            info = {
                **base_info,
                ...
                "phone_number": ensure_unicode(chat_obj.phone_number)
                if hasattr(chat_obj, "phone_number")
                else None,
                ...
                if hasattr(chat_obj, "language_code")
                else None,
            }
...
@mcp.tool()
async def get_contacts(ctx: Context | None = None) -> str:
            ...
            contacts.append(
                {
                    ...
                    "phone_number": ensure_unicode(user.phone_number)
                    if hasattr(user, "phone_number")
                    else None,
                    "is_contact": True,
                    "is_mutual_contact": user.is_mutual_contact
                    if hasattr(user, "is_mutual_contact")
                    else False,
...
async def search_in_chat(
    chat: str, query: str, limit: int = 100, ctx: Context | None = None
) -> str:
...
async def get_chat_list(
    ...
    try:
        ...
        logger.debug(
            f"Getting dialogs with: limit={limit}, offset_id={offset_id_int}, offset_date={offset_date_ts}"
        )
        async for dialog in mcp.client.get_dialogs(limit=limit):
            ...
            if (
                ...
                and dialog.top_message.date.timestamp() >= offset_date_ts
            ):
                continue
            ...
            info = {
                ...
                "first_name": ensure_unicode(chat.first_name)
                ...
                "latest_message": serialize_message(dialog.top_message)
...
@mcp.tool()
async def get_media_content(file_id: str, ctx: Context | None = None) -> MCPImage:
...
@mcp.tool()
async def get_common_chats(user: str, ctx: Context | None = None) -> str:
